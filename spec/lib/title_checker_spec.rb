...
describe TitleChecker do
  ...
  context 'under expected conditions gives title' do
    ...
    it 'Apprenctice Wordsmith' do
      expect(tc.apprentice_wordsmith).to be_true
    end
    ...
    it 'Lord Tennyson' do
      ...
      expect(tc.lord_tennyson).to be_true
    end

    it 'Let Us Go' do
      poem.text = 'not much here'
      expect(tc.let_us_go).to be_true
    end

    it 'Wordsworthy' do
      user.word_count = 100
      expect(tc.wordsworthy).to be_true
    end

    it 'Tweet Brigade' do
      user.word_count = 600
      expect(tc.tweet_brigade).to be_true
    end

    it 'Byronic Hero' do
      poem.text = 'stormy moors'
      expect(tc.byronic_hero).to be_true
    end

    it 'Poe-Trie' do
      poem.text = 'Once upon a Midnight DrEaRy'
      expect(tc.poe_trie).to be_true
    end

    it 'Lovecraftian' do
      poem.text = 'hail to Cthulhu'
      expect(tc.lovecraftian).to be_true
    end
    ...
    it 'Duke of Repartee' do
      ...
      expect(tc.duke_of_repartee).to be_true
    end

    it 'Rake Progressing' do
      poem.text = 'Shite, I gotta get me out o Liverpool'
      expect(tc.rake_progressing).to be_true
    end
